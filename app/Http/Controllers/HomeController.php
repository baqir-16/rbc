...
class HomeController extends Controller
{
    ...
    // Returns dashboard information of this OpCo
    public function index()
    {
        // Home Dashboard 
        if(Auth::user()->roles->contains('1')) {

        $updated_date = $critical_open = DB::connection('mongodb')->collection('db_num_of_hosts_by_cat')->select("updated_at")->where('department', null)
...
//  ======================
        ...
        $db_opco_findings = DB::connection('mongodb')->collection('db_opco_findings')->where('department', null)->first();
        ...
        return view('home', compact('totalTime','usercount', 'ticketscount','streamscount', 'st_complete',
            ...
            $updated_date = $critical_open = DB::connection('mongodb')->collection('db_num_of_hosts_by_cat')->where('department', Auth::user()->department)->select("updated_at")
            ->whereNotNull('updated_at')
            ->first();
...
//  ======================
        ...
        $db_opco_findings = DB::connection('mongodb')->collection('db_opco_findings')->where('department', Auth::user()->department)->first();
        $opco_risk_array = $db_opco_findings['opco_risk_array'];
        $opco_array = $db_opco_findings['opco_array'];

        $db_open_close_findings = DB::connection('mongodb')->collection('db_open_close_findings')->where('department', Auth::user()->department)->first();
        $open = $db_open_close_findings['open'];
        $close = $db_open_close_findings['close'];

        $db_num_of_hosts_by_cat = DB::connection('mongodb')->collection('db_num_of_hosts_by_cat')->where('department', Auth::user()->department)->first();
        $cat_array = $db_num_of_hosts_by_cat['cat_array'];
        $hosts_per_cat_array = $db_num_of_hosts_by_cat['hosts_per_cat_array'];
        ...
        $db_num_of_hosts_by_vuln_name = DB::connection('mongodb')->collection('db_num_of_hosts_by_vuln_name')->where('department', Auth::user()->department)->first();
        $num_of_hosts_by_vuln_name = $db_num_of_hosts_by_vuln_name['num_of_hosts_by_vuln_name'];
        ...
        $high_risk = DB::connection('mongodb')->collection('scan_results')->whereNotNull('hod_signoff_date')->where('rem_officer_rem_status', 0)->where('rem_pmo_closure_status', 0)->where('false_positive', 0)
            ->where('risk', (int)Config::get('enums.severity_status.High'))->where('department', Auth::user()->department)->count();

        $critical_risk = DB::connection('mongodb')->collection('scan_results')->whereNotNull('hod_signoff_date')->where('rem_officer_rem_status', 0)->where('rem_pmo_closure_status', 0)->where('false_positive', 0)
            ->where('risk', (int)Config::get('enums.severity_status.Critical'))->where('department', Auth::user()->department)->count();
    ...
    // Returns findings by severity by this OpCo
    public function riskdetails($id) {
        ...
        if($id == 4)
            $low_risk_dt = DB::connection('mongodb')->collection('scan_results')
                ->where('risk', (int)Config::get('enums.severity_status.Low'))->whereNotNull('hod_signoff_date')->where('rem_officer_rem_status', 0)->where('rem_pmo_closure_status', 0)->where('department', Auth::user()->department)->where('false_positive', 0)->get();
        elseif ($id == 3)
            $med_risk_dt = DB::connection('mongodb')->collection('scan_results')
                ->where('risk', (int)Config::get('enums.severity_status.Medium'))->whereNotNull('hod_signoff_date')->where('rem_officer_rem_status', 0)->where('rem_pmo_closure_status', 0)->where('department', Auth::user()->department)->where('false_positive', 0)->get();
        elseif ($id == 2)
            $high_risk_dt = DB::connection('mongodb')->collection('scan_results')->whereNotNull('hod_signoff_date')->where('rem_officer_rem_status', 0)->where('rem_pmo_closure_status', 0)->where('department', Auth::user()->department)->where('false_positive', 0)
                ->where('risk', (int)Config::get('enums.severity_status.High'))->get();
        elseif ($id == 1)
            $critical_risk_dt = DB::connection('mongodb')->collection('scan_results')->whereNotNull('hod_signoff_date')->where('rem_officer_rem_status', 0)->where('rem_pmo_closure_status', 0)->where('department', Auth::user()->department)->where('false_positive', 0)
                ->where('risk', (int)Config::get('enums.severity_status.Critical'))->get();
        else
            $pending_rem_dt = DB::connection('mongodb')->collection('scan_results')->whereNotNull('hod_signoff_date')->where('rem_officer_rem_status', 1)->where('rem_pmo_closure_status', 0)->where('department', Auth::user()->department)->where('false_positive', 0)->get();

        return view('home.showdetails', compact('med_risk_dt', 'high_risk_dt', 'low_risk_dt','critical_risk_dt', 'pending_rem_dt', 'enums','id','opcos'));
    }
}

    // Returns total count of findings by opco
    public function opco_findings()
    {
        ...
        $db_opco_findings = DB::connection('mongodb')->collection('db_opco_findings')->get();
 ...
 $departments = DB::table('departments')->pluck('id')->toArray();
        ...
        foreach($opco_array as $opco){
            ...
            $critical = DB::connection('mongodb')->collection('scan_results')
                ->whereNotNull('hod_signoff_date')
                ->where('rem_officer_rem_status', 0)->where('rem_pmo_closure_status', 0)
                ->where('false_positive', 0)->where('department', (int)$dep)
                ->where('risk', (int)Config::get('enums.severity_status.Critical'))
                ->where('opco_id', (int)$opco['id'])
                ->count();
        ...
        $bool = DB::connection('mongodb')->collection('db_opco_findings')->where('department', (int)$dep)->count();
        $check = DB::connection('mongodb')->collection('db_opco_findings')->where('department', (int)$dep)->get();

        if($bool != null) {
            DB::connection('mongodb')->collection('db_opco_findings')
            ->where('department', (int)$dep)
            ->update([
                'opco_risk_array' => $opco_risk_array,
                'opco_array' => $opco_array,
                'updated_at' => new UTCDateTime(strtotime(date("Y/m/d H:i:s"))*1000),
    ...
    // Returns findings count by severity
    public function risk_levels()
    {
        ...
        $db_risk_levels = DB::connection('mongodb')->collection('db_risk_levels')->get();
 ...
 $departments = DB::table('departments')->pluck('id')->toArray();
        ...
        $info_risk = DB::connection('mongodb')->collection('scan_results')->whereNotNull('hod_signoff_date')->where('rem_officer_rem_status', 0)->where('rem_pmo_closure_status', 0)->where('false_positive', 0)->where('department', (int)$dep)
            ->where('risk', (int)Config::get('enums.severity_status.Informational'))->count();

        $low_risk = DB::connection('mongodb')->collection('scan_results')->whereNotNull('hod_signoff_date')->where('rem_officer_rem_status', 0)->where('rem_pmo_closure_status', 0)->where('false_positive', 0)->where('department', (int)$dep)
            ->where('risk', (int)Config::get('enums.severity_status.Low'))->groupby('name', 'host')->count();

        $med_risk = DB::connection('mongodb')->collection('scan_results')->whereNotNull('hod_signoff_date')->where('rem_officer_rem_status', 0)->where('rem_pmo_closure_status', 0)->where('false_positive', 0)->where('department', (int)$dep)
            ->where('risk', (int)Config::get('enums.severity_status.Medium'))->groupby('name', 'host')->count();

        $high_risk = DB::connection('mongodb')->collection('scan_results')->whereNotNull('hod_signoff_date')->where('rem_officer_rem_status', 0)->where('rem_pmo_closure_status', 0)->where('false_positive', 0)->where('department', (int)$dep)
            ->where('risk', (int)Config::get('enums.severity_status.High'))->count();

        $critical_risk = DB::connection('mongodb')->collection('scan_results')->whereNotNull('hod_signoff_date')->where('rem_officer_rem_status', 0)->where('rem_pmo_closure_status', 0)->where('false_positive', 0)->where('department', (int)$dep)
            ->where('risk', (int)Config::get('enums.severity_status.Critical'))->count();


        $bool = DB::connection('mongodb')->collection('db_risk_levels')->where('department', (int)$dep)->count();
        $check = DB::connection('mongodb')->collection('db_risk_levels')->where('department', (int)$dep)->get();
    ...
    // Returns total number of open and closed findings
    public function open_close()
    {
        ...
        $db_open_close_findings = DB::connection('mongodb')->collection('db_open_close_findings')->get();
       ...
       $close = DB::connection('mongodb')->collection('scan_results')->where('department', (int)$dep)
            ->where('rem_pmo_closure_status', 1)->where('false_positive', 0)->whereNotNull('hod_signoff_date')->count();
        $open = DB::connection('mongodb')->collection('scan_results')->where('department', (int)$dep)
            ->where('rem_officer_rem_status', 0)->where('rem_pmo_closure_status', 0)->where('false_positive', 0)->whereNotNull('hod_signoff_date')->count();

        $bool = DB::connection('mongodb')->collection('db_open_close_findings')->where('department', (int)$dep)->count();
        $check = DB::connection('mongodb')->collection('db_open_close_findings')->where('department', (int)$dep)->get();
    ...
    // Returns number of hosts by category
    public function num_of_hosts_by_cat()
    {
        ...
        $db_num_of_hosts_by_cat = DB::connection('mongodb')->collection('db_num_of_hosts_by_cat')->get();
        ...
        foreach($cat_array as $key=>$cat){
         ...
         $bool = DB::connection('mongodb')->collection('db_num_of_hosts_by_cat')->where('department', (int)$dep)->count();
        $check = DB::connection('mongodb')->collection('db_num_of_hosts_by_cat')->where('department', (int)$dep)->get();
    ...
    // Returns number of hosts by vulnerability name
    public function num_of_hosts_by_vuln_name()
    {
        ...
        $db_num_of_hosts_by_vuln_name = DB::connection('mongodb')->collection('db_num_of_hosts_by_vuln_name')->get();
        ...
        $bool = DB::connection('mongodb')->collection('db_num_of_hosts_by_vuln_name')->where('department', (int)$dep)->count();
        $check = DB::connection('mongodb')->collection('db_num_of_hosts_by_vuln_name')->where('department', (int)$dep)->get();
